// src/components/calculations/VcCalculator.tsx
...
export const VcCalculator: React.FC<VcCalculatorProps> = ({ onBack, engine }) => {
  ...
  useEffect(() => {
    ...
    if (mode === 'codec') { config.codecPreset = codecPreset; config.resolution = resolution; config.fps = fps; config.participants = participants; }
    else { config.multipointParticipants = multipointParticipants; }
    dispatch(setVcConfig(config));
  }, [mode, codecPreset, resolution, fps, participants, multipointParticipants, dispatch]);

  useEffect(() => { setResult({ value: vc.resultValue, text: vc.resultText }); }, [vc]);
  ...
  return (
    <div className="calculator-container">
      ...
      <div className="calculator-form">
        <div className="setting"><label>Режим:</label><select value={mode} onChange={e => setMode(e.target.value as any)}><option value="codec">Расчёт битрейта кодеков</option><option value="multipoint">Многоточечный вызов</option></select></div>
        {mode === 'codec' && (<><div className="setting"><label>Кодек:</label><select value={codecPreset} onChange={e => setCodecPreset(e.target.value)}><option value="trueconf">TrueConf</option><option value="webrtc">WebRTC</option><option value="h264">H.264</option><option value="h265">H.265</option></select></div><div className="setting"><label>Разрешение:</label><select value={resolution} onChange={e => setResolution(e.target.value)}><option value="720p">720p</option><option value="1080p">1080p</option><option value="4K">4K</option></select></div><div className="setting"><label>FPS:</label><input type="number" value={fps} onChange={e => setFps(parseInt(e.target.value))} /></div><div className="setting"><label>Количество участников:</label><input type="number" value={participants} onChange={e => setParticipants(parseInt(e.target.value))} /></div></>)}
        {mode === 'multipoint' && (<div className="setting"><label>Количество участников:</label><input type="number" value={multipointParticipants} onChange={e => setMultipointParticipants(parseInt(e.target.value))} /></div>)}
